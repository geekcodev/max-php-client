...
final class Uploader
{
    ...
    /**
     * Multipart-тело собирается в seekable php://temp-поток: файл копируется по чанкам
     * (потоковая передача, без загрузки в память целиком), а seekable-поток безопасен
     * для повторов запроса при ретраях.
     */
    private function multipart(string $boundary, string $fileName, string $contentType, string $filePath): StreamInterface
    {
        $crlf = "\r\n";
